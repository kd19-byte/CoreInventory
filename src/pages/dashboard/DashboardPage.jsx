  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [
        ...
        supabase.from('products').select('*', { count: 'exact', head: true })
          .gt('reorder_point', 0).filter('current_stock', 'lte', 'reorder_point'),
        ...
        supabase.from('receipts').select('id,reference,status,scheduled_date').order('created_at', { ascending: false }).limit(5),
        supabase.from('delivery_orders').select('id,reference,status,scheduled_date').order('created_at', { ascending: false }).limit(5),
      ])

      setKpis({ total, lowStock, outOfStock, pendingReceipts, pendingDeliveries })
